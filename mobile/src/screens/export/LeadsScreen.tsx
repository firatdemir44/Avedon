...
export function LeadsScreen({ navigation }: Props) {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: t.colors.surface0 }}>
      <AppBar title={tr('Takip listem')} leading="back" onBack={() => navigation.goBack()} />
      <ScrollView
      ...
      >
        {error ? (
          <EmptyState icon="warning" title={tr('Liste alınamadı')} description={error} actionLabel={tr('Tekrar dene')} onAction={() => load()} />
        ) : leads === null ? (
          ...
          <EmptyState
            ...
            title={tr('Takip ettiğiniz alıcı yok')}
            description={tr("Dünyayı Keşfet'te bir ülkenin aday alıcılarından birini 'Takibe al' ile buraya ekleyin.")}
            actionLabel={tr('Dünyayı Keşfet')}
            onAction={() => navigation.navigate('ExportRadar')}
          />
        ) : (
          groups.map((g) => (
            <View key={g.key} style={{ gap: t.space[3] }}>
              <SectionTitle title={`${tr(g.label)} · ${g.items.length}`} />
              {g.items.map((l) => (
                <Card key={l.id}>
                  <View style={{ gap: t.space[2] }}>
                    ...
                    <ButtonRow>
                      ...
                      <Button
                        kind="quiet"
                        icon="open-outline"
                        label={l.buyer.website ? tr('Web sitesi') : tr('Sicil kaydı')}
