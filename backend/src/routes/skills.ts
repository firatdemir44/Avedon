...
skillsRouter.get('/', (req, res) => {
  res.json({ skills: listSkills(req.lang) });
});
...
skillsRouter.post('/:name/run', async (req, res) => {
  ...
  const result = runSkill(skill, body, req.lang);
  if (!result.ok) return res.status(400).json({ error: result.error, details: result.details });

  res.json({ skill: skill.name, title: t(req.lang, skill.title), output: result.output, summary: result.summary, formula: skill.formula });
});
