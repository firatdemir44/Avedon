...
<nav class="tabbar">     64px, --surface-1, üst kenarlık --line; 5 sekme: Ana sayfa · Katalog · Asistan · Mesajlar · Firmalar (Asistan simgesi büyük Takyon işareti)
```
...
| Makine tablosu (Makineler sekmesi, varsayılan) | Firmanın kendi parkur tablosu gibi, makine başına bir satır. Üstte özet `.body-14` `--ink-2` (sayılar `.mono-14` `--ink`): "38 makine · 24 müsait · 14 dolu" (adetli kayıt adediyle sayılır); sağında segment "Tablo | Kart" (Kart = makine kartları). Altında tek çip satırı: Tümü · Müsait · Dolu · örgü cinsleri (veriden). Sütunlar: No · Pus · Fine · Marka · İğne · Sistem · Örgü cinsi · Durum; satır `--table-row` 44px, 1px `--line` ayırıcı (zebra yok), başlık satırı `--surface-2` + `.label-14` `--ink-2` ve tablonun kendi dikey kaydırmasında (en çok ekranın %70'i) üstte sabit. Sayılar `.mono-14`, metin `.body-14`, boş hücre "–" `--ink-3`; hücre metni tek satır. Dar ekranda **No solda, Durum sağda sabit**, aradaki sütunlar birlikte yatay kaydırılır (sayfa değil, yalnız tablo gövdesi). Fine/İğne aralık olabilir ("28–22", dönüştürülebilir makine). Durum: 8px nokta + `.label-14` kısa metin, makine kartıyla aynı renk kuralı: "Müsait" `--success` · 14 güne kadar "5 gün" (≤7 `--warning`, sonrası `--danger`) · daha uzun "12 Eki" (§5 ay kısaltması). Sıra: No, sonra eklenme. Sahibi satıra dokununca düzenler, Durum hücresine dokununca müsaitlik alt sayfası; ziyaretçi dokununca makine kartı alt sayfada. Sahibe kenarlıklı "Makine ekle" · "Fotoğraftan aktar" · "Kapasite". **Fotoğraftan aktar**: fotoğraf/PDF seç → "Okunuyor…" iskelet → aynı tablo önizleme (Durum yerine "Kayıt": Yeni / Var, atla (`--warning`) / Düzelt (`--danger`)); satıra dokununca alt sayfada düzelt/sil; parkurda aynı No varsa uyarı şeridi + "Aynı numaralıları atla" çipi (varsayılan açık); yapışkan ana düğme "N makineyi ekle". |
| Segment kontrol | `--surface-2` zemin 4px iç boşluk `--radius-md`; öğe 36px; seçili `--surface-1` + 1px `--line`. |
| Sekme çubuğu | 64px; 5 sekme: Ana sayfa · Katalog · Asistan · Mesajlar · Firmalar. Asistan ortada; simgesi renkli Takyon işareti, `size.tabIconLg` (34px, ~1.4×) — ikon kutusu 24px kalır, etiket hizası bozulmaz. Diğer ikonlar 24px `--icon` + `.caption-12` etiket her zaman birlikte; aktif sekme: `--brand-soft` zemin + üstte 2px `--brand` çizgi + `--brand` ikon/metin; pasif `--ink-3`; bildirim noktası 8px `--accent`. Talepler sekme değil: ana sayfadaki "Talepler" kısayolundan açılan geri oklu sayfa; bekleyen numune/yeni teklif varsa kısayolda accent nokta. Hesap araçları sekme değil: ana sayfadaki "Hesap araçları" kısayolundan açılan, geri oklu üst bantlı ayrı sayfa; "Dış pazar" da bu sayfanın içinde. |
...
| Kısayol kutusu (Ana sayfa) | Dünyayı Keşfet · Teklif iste · Hesap araçları · Talepler (numune kutusu ikonu; bekleyen varsa 8px `--accent` nokta). 80px, `--surface-1` kart; sol 40px ikon karesi `--brand-soft`/`--brand` `--radius-md`; metin 16px 600, iki satıra kırılabilir. 2 sütun, aralık `--space-3`. |
