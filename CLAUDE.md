...
**Skill paketi (Fırat 2026-09-26):** Bu proje için önce `takyon-yol-haritasi` skill'ini oku; hangi aşamada olduğumuzu oradan öğren ve ilgili aşama skill'ini kullan (`.claude/skills/`).
...
- **Önce örnek (Fırat 2026-09-26):** toplu işleme (ör. Texart fotoğrafları) önce 1-2 örnekle gösterilir, onaydan sonra tamamı çalıştırılır.
...
- **Zaman planı `docs/global-yol-haritasi.md`** (kısa/orta/uzun vade, aşama çıkış ölçütleri; Fırat onayı 2026-09-26). Haftalık ritim: pazartesi 3 iş, cuma rapor, ay sonu ölçüt kontrolü.
...
- **Model seçimi — işe en uygun model (Fırat 2026-09-26: "hangi versiyon o işe yatkınsa onunla; Haiku'nun yapacağı işi Fable'a yaptırmayalım"):** üç kademe: **Haiku 4.5** basit ve mekanik işler (dosya taşıma/adlandırma, test fotoğrafı ekleme, çeviri sözlüğü girişleri, metin düzeltme, arama/tarama); **Opus 5** rutin kodlama, ekran/form, test, doküman, yayın takibi; **Fable 5.1** yalnızca veri modeli/migration, mimari/plan, kök neden ve ağır algoritma (Texart görüntü işleme). Kademe geçişi alt görevle (Agent, model: haiku/opus/fable) yapılır, sonuç ana oturumda doğrulanır.
- **Model seçimi (ilk karar, kullanıcı kararı, 2026-09-16: "gerekli yerde Fable 5.1, diğer alanlarda Opus 5; buna kendin karar ver"):** veri modeli/migration tasarımı, mimari ve plan yazımı, çok dosyaya dokunan yeniden yapılandırmalar ve kök neden araştırması **Fable 5.1**; rutin kodlama, ekran/form işleri, test yazımı, doküman güncellemeleri, yayın takibi **Opus 5**. Asistan geçişe kendisi karar verir ve ilgili işin başında hangi modelle çalıştığını kısaca söyler. **Mekanik:** ana oturumun modelini asistan kendisi değiştiremez (araç bunu reddediyor); ya kullanıcıdan /model ile geçiş ister ya da o parçayı uygun modelde çalışan bir alt göreve (Agent, model: opus/fable) verir ve sonucu kendisi doğrular (tsc, testler, tarayıcı).
