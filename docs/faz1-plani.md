...
**Kaynak:** `docs/yol-haritasi.md` §6 Faz 1 (vizyon §4 kumaş pasaportu, §5 asistan ve beceriler, §7 teknik notlar) · **Tarih:** 2026-09-16 · **Durum:** onaylandı; Adım 1 ve Adım 2 tamamlandı ve canlıda (2026-09-16); Adım 3 ve Adım 4 tamamlandı; Adım 5 ve Adım 6 tamamlandı (2026-09-16; tarayıcıda doğrulandı, telefonda denenmedi); Adım 7 kod tarafı tamamlandı (Meta hesabı bağlanınca canlı test); Adım 9 (İpek / Mert) tamamlandı (2026-09-16, tarayıcıda doğrulandı); kalan: Adım 8 (SMS, sağlayıcı seçimi bekliyor). Hazırlayan: Claude (Fable 5.1), mevcut kod okunarak.
...
- **Fırat'ın cevapları (2026-09-16) ve uygulanması:**
  1. *Randıman / günlük saat / makine adedi:* varsayılan olur ama asistan her seferinde sormaz; firma ayarlarından (firma hafızası) gelir. Önerilen başlangıç %85, 20 saat, 1 makine (sektör standardı değil, hesaba otomatik girmez). → `memoryKeys.ts`'e `efficiencyPercent`, `hoursPerDay`, `machineCount` (+ `suggested`), `knitProduction` beceriye `machineCount` ve satır adı `label`; randıman/saat artık şemada varsayılansız (hafızadan ya da sorulur). Sistem talimatı: sormadan hafızadan kullan, yoksa bir kez sor ve kaydetmeyi öner.
  2. *Tüp en (ek bilgi 2026-09-17: tüp en genellikle yalnızca HAM kumaşta olur, boyalı/mamul kumaş büyük çoğunlukla açık endir; formda varsayılan "Açık en", asistan belirtilmemişse açık en varsayar):* tek yüz tüp eni ise açık en = 2×; ama otomatik çarpma her zaman doğru değil, bazı işletmeler tüp kumaşı açık enle söyler. → `fabricLengthWeight` ve `yarnUsage` becerilerine `widthMeaning: acik | tup_tek_yuz`; asistan tüp denince sorar. Ürün pasaportunda "enin anlamı" alanı ve hesapta kullanılacak en (açık iş, Adım 6+).
  3. *Konfeksiyon kalemleri:* kesinlikle ayrı: kumaş, kesim, dikim, yıkama/baskı/boya, aksesuar (fermuar, düğme, lastik, etiket, ip, tela, kordon, metal), paketleme, nakliye, genel gider/fire/diğer. Örnek sweatshirt 180+15+65+25+12+8+10 = 315 ₺ (varsayımsal). → `garmentCost` beceri kalem kalem; `formulas.ts` kopyası değişmedi (kumaş maliyeti ondan, kalemler beceri katmanında toplanır). Mobil `GarmentCostCalculator` ekranı da 2026-09-17'de kalem kalem oldu (`mobile/src/features/calculators/garmentItems.ts`; para birimi seçimi, sipariş adedi, yüzde payı ve en büyük kalem vurgusu).
  4. *Fireler ayrı:* örme (iplik→ham), boya/apre (ham→mamul), kesim (mamul→dikilmiş ürün); zincir: ihtiyaç = sonraki / (1−fire). Örnek: 1.000 kg mamul, boya %5, örme %3 → ham 1.052,63, iplik 1.085,19 kg. → yeni `domain/calc/wastage.ts` (`requirementChain`) + beceri `yarnRequirement`; hafıza anahtarları `knittingLossPercent`, `cuttingLossPercent`; `yarnUsage` tek fireli hızlı hesap olarak kaldı.
  5. *Ham → mamul gramaj:* sabit yüzde yok; en değişimi ve relaksasyon belirleyici (320 g/m: 180 cm → 177,8, 160 cm → 200 g/m²). Üç düzey: A teorik ham, B tahmini mamul (en değişimi ya da kullanıcının geçmiş oranı), C gerçek ölçüm; sapma C'den hesaplanır. → `fabricGsmKnit` beceriye `greigeWidthCm/finishedWidthCm`, `finishChangePercent`, `measuredFinishedGsm`; ürün pasaportuna A/B/C gramaj alanları (açık iş).
  6. Gerçek örnekler: yukarıdaki üç örnek testlere işlendi (`wastage.test.ts`, `skills.test.ts`); hesap başına 2-3 örnek hâlâ bekleniyor.
  Testler: birim 113/113, beceri API 39/39.
...
### Ne yapılacak
- `routes/whatsappWebhook.ts` (bugün 24 satır, yalnızca loglar): (1) **imza doğrulaması** `X-Hub-Signature-256` (Meta uygulama gizli anahtarı `WHATSAPP_APP_SECRET`; bugün yok, açık bir güvenlik boşluğu), (2) hemen 200 döner, işlemeyi arka planda yapar (Meta 200 gelmezse yeniden gönderir), (3) mesaj kimliğiyle **tekrar koruması** (`WhatsAppInbound (messageId unique, from, body, receivedAt, status)`), (4) göndereni telefon numarasından bulur: Meta `90XXXXXXXXXX` verir, `User.phone` `0XXXXXXXXXX` biçiminde; `whatsapp.ts`'teki `toE164` mantığının tersi tek yerde (`phone.ts`) yazılır, (5) kullanıcı bulunursa `channel: whatsapp` ipliğinde asistanı çağırır, cevabı `sendWhatsAppText` ile gönderir (kullanıcı yazdığı için 24 saat penceresi açık, şablon gerekmez), (6) kullanıcı yoksa kısa bir "Takyon Ai'a kayıt olun" cevabı ve bağlantı.
